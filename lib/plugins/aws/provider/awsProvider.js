...
class AwsProvider {
  ...
  constructor(serverless, options) {
    ...
    console.log("AWSPROVIDER PROXY IS " + proxy);
    new ffs(url.parse(proxy));
    console.log("afterffs PROXY IS " + proxy);

    if (proxy) {
      console.log("process.env.NO_PROXY = " + process.env.NO_PROXY);
      console.log("process.env.no_proxy = " + process.env.no_proxy);
      // AWS.config.httpOptions.agent = new HttpsProxyAgent(url.parse(proxy));
      AWS.config.httpOptions.agent = new ffs(url.parse(proxy));

      //AWS.NodeHttpClient.sslAgent = new https.Agent({rejectUnauthorized: true});

      //AWS.config.httpOptions.proxy = proxy;
    }

    console.log("PROXY bypassed " + proxy);
  ...
  request(service, method, params, options) {
    ...
    const request = this.requestQueue.add(() => persistentRequest(() => {
      ...
      console.log("awsService method " + method);
      console.log("awsService params " + JSON.stringify(params,0,4));

      const req = awsService[method](params);

      console.log(" req keys " + Object.keys(req));

// domain,service,operation,params,httpRequest,startTime,response,_asm,_haltHandlersOnError,_events,emit

      if ( req.httpRequest ) {
        console.log(" req domain " + req.domain);
        console.log(" req service " + req.service);
        console.log(" req httpRequest " + JSON.stringify(req.httpRequest));
        console.log(" req httpRequest.endpoint.hostname " + req.httpRequest.endpoint.hostname);
        console.log(" req httpRequest.endpoint.href " +  req.httpRequest.endpoint.href);
  
      //   var endpoint = req.httpRequest.endpoint.hostname; 
      //   if ( endpoint == "s3.ap-southeast-2.amazonaws.com" || endpoint == "xxxcloudformation.ap-southeast-2.amazonaws.com") {
      //     console.log("attempting to hacky remove proxy ");
      //     that.oldAgent =  AWS.config.httpOptions.agent;
      //     AWS.config.httpOptions.agent = null;
      //   } else {
      //     console.log("using proxy (though it may have been hackily removed) using " + that.oldAgent);
      //     AWS.config.httpOptions.agent = that.oldAgent;
      //   }
      }


      //console.log("sending req " + JSON.stringify(req,0,4));
